...
public class DP_CountWays_357 {
	...
	public static void main(String[] args) {
		...
		while((str = reader.nextLine())!=null && str.trim().length() >0 &&  (input = Integer.parseInt(str.trim()))>=0){
			...
			if(output ==1){
				pw.println("There is only 1 way to produce "+input+" cents change.");
			}else
			pw.println("There are "+output+" ways to produce "+input+" cents change.");
		}
		pw.close();
